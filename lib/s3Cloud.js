...
class S3Cloud extends Cloud {
    ...
    async destroyFile( destPath ) {
        console.log('In a promise'+ destPath);

        let pathType = Utils.getPathType(destPath);

        if (pathType === 'root') {
            throw 'Error: Path ' + destPath + ' is not a valid delete file target.';
        }
        else if (pathType === 'rootDir') {
            throw 'Error: Path ' + destPath + ' is not a valid delete file target.';
        }
    ...
    async emptyDir( destPath ) {

        let pathType = Utils.getPathType(destPath);

        if (pathType === 'root') {
            throw 'Error: Path ' + destPath + ' is not a valid delete dir target.';
        }
        else if (pathType === 'rootDir') {
            throw 'Error: Path ' + destPath + ' is not a valid delete dir target.';
        }
    ...
    async emptyRootDir( destPath ) {
        ...
        if (pathType === 'root') {
            ...
            throw 'Error: Path ' + destPath + ' is not a valid empty root dir target.'
        }
        else if (pathType === 'file') {
            throw 'Error: Path ' + destPath + ' is not a valid delete dir target.';
        }
    ...
    async destroyRootDir( destPath ) {

        let pathType = Utils.getPathType(destPath);

        if (pathType === 'root') {
            throw 'Error: Path ' + destPath + ' is not a valid delete root dir target.';
        }
        else if (pathType === 'rootDir') {
            let container = Utils.parseContainerFromPath(destPath);
            let filePath = Utils.removeRootDirFromPath(destPath);

            let delList = await this.ls(destPath);
    ...
    async copyFile( srcPath, destPath ) {
        ...
        if (pathType === 'root') {
            throw 'Error: Path ' + destPath + ' is not a valid delete copy file target.';
        }
        else if (pathType === 'rootDir') {
            throw 'Error: Path ' + destPath + ' is not a valid delete copy file target.';
        }
            ...
            try {
                do{
                    let destValid = await this.fileExists(destPath);
                    console.log('COPY FILE', destValid);
                }while(destValid === true);
                //if destpath already exists
                    //check for destPath_1,2,3,4 etc until one works
                    //if one works
                        //create write
                        //pushdata from read to dest write
